import type { Player, RawPlayerData } from "../../../shared/aaa"
...
const fetchDataFromAPI = async (client: PoolClient) => {
    ...
    try {
        responsePlayers = await nodeFetch(
            "https://play.retro-mmo.com/players.json"
        )
        responseLeaderboards = await nodeFetch(
            "https://play.retro-mmo.com/leaderboards.json"
        )
    } catch (err) {
    ...
    for (const [index, player] of Object.entries(data)) {
        ...
        if (current_leaderboards[index] != null) {
            ...
            if (experience_diff !== 0) {
                logger.info(
                    `${_player.username} gained ${experience_diff} experience.`
                )
            }
        }
