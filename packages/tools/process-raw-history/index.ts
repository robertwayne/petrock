...
const processRawHistoryFile = async (client: PoolClient) => {
    const data = readFileSync('./packages/tools/process-raw-history/raw_history.json', 'utf-8')
    ...
    for (const [_, player] of Object.entries(parsed)) {
        ...
        for (const [i, record] of Object.entries(history).reverse()) {
            ...
            if (index === 0) {
                ...
                _player.experience = record.experience - player.history[index - 1].experience
            }

            _player.created_on = new Date(record.time)

            console.log(`Starting query ${index + 1}/${player.history.length} for ${_player.username}.`)
            await client.query({
                ...
                DO NOTHING;
                `,
                values: [_player.username, _player.experience, _player.created_on],
            })
